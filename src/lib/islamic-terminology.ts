...
- **This holds in EVERY target script, not just Latin-alphabet ones.** When the target language does not use the Latin alphabet, TRANSLITERATE the name into that script. NEVER substitute the local generic word for God — that is exactly the flattening this rule exists to prevent, and it is just as wrong in Hebrew or Greek as writing "God" in English. Correct forms: Arabic الله · Hebrew אללה (NEVER אלוהים / אלהים / אל) · Greek Αλλάχ (NEVER Θεός / Θεό / Θεώ) · Russian and Ukrainian Аллах (NEVER Бог) · Hindi अल्लाह (NEVER भगवान / ईश्वर) · Japanese アッラー — katakana ONLY, and never any Chinese-character rendering of the name · Korean 알라 — hangul ONLY, and never any Chinese-character rendering · Chinese (and only Chinese) 真主 or 安拉, never 上帝. The same rule governs every other divine name, honorific, and formula below: transliterate into the target script, never swap in the local religion-neutral equivalent.
- The ban on the local word for God applies to Latin-alphabet languages too, and to EVERY mention in the sentence — not just the first. Write Allah (or the language's standard Islamic spelling, e.g. Alá / Al·là / Alláh / Allahin / Allahowi with normal inflection) and NEVER: Isten (Hungarian) · Gud (Danish/Norwegian/Swedish) · Jumala (Finnish) · Bóg/Bogu (Polish) · Bůh/Bohu (Czech) · Dumnezeu (Romanian) · Déu (Catalan) · Dios · Dieu · Gott · Deus · Dio · God · Tuhan used alone as a substitute. If a sentence mentions Allah three times, all three must be Allah.
- Keep the output in ONE script — the target language's own. Do not splice in words from another writing system (e.g. no Chinese 真主 inside Japanese or Korean text, no Latin-letter "Allah" inside Greek or Cyrillic text — write Αλλάχ / Аллах).
- **PRESERVED TERMS ARE EMBEDDED IN TARGET-LANGUAGE PROSE — they never replace the translation.** Keeping "Allah", "Alhamdulillah", "Insha'Allah", "Rabb", honorifics, etc. does NOT mean leaving the sentence untranslated. Every word that is not itself a preserved Islamic term MUST be rendered in the target language. Two failures to avoid absolutely: (a) outputting a romanized transliteration of the Arabic in place of a translation — e.g. for Swedish, "Alhamdu lillahi Rabbil 'aalameen wa sallallahu 'ala nabiyyina Muhammad" is WRONG, it must read as Swedish ("All lov tillkommer Allah, världarnas Herre, och må Allahs frid och välsignelser vara över vår profet Muhammad ﷺ"); and (b) falling back to English when the target is some other language — if the target is Norwegian, "Lord of the worlds" is WRONG, write "verdenes Herre". The output must read as fluent, natural TARGET-language text to a native speaker of it.
- Output the translation EXACTLY ONCE. Do not prepend or append a romanized transliteration of the source sentence (e.g. do not write "Alhamdu lillahi Rabbil 'aalameen wa sallallahu 'ala nabiyyina Muhammad." before or after the target-language rendering) — the reader sees the original in the other pane already, so a duplicate is noise.
- Write the MODERN standard register of the target language. GREEK SPECIFICALLY: write monotonic MODERN Greek (one acute accent only, e.g. "Ο έπαινος ανήκει στον Αλλάχ"); never Ancient/Koine grammar and never polytonic diacritics (no ῷ ᾶ ὁ ἀ ἔ ῳ — those mark Ancient Greek and read as a liturgical antique to a modern speaker). HEBREW: modern Israeli Hebrew, not Biblical. Religious vocabulary is preserved; the surrounding grammar must read as contemporary speech.
