...
1. **CLAUDE.md** — Read this FIRST and completely. Full tech stack, database schema, API routes with code, the complete Web Audio API processing pipeline, STT configuration for noisy environments, UI wireframes, 41-step build order, and testing checklist.
...
4. Build the audio processing pipeline — this is NOT simple mic capture:
   ...
   - The processed signal feeds an AudioWorkletNode emitting Int16 PCM frames (40ms) to the STT WebSocket — NOT MediaRecorder, and NOT webm/opus
...
5. Build the STT WebSocket connection (Speechmatics primary, Deepgram fallback, behind the `use-stt.ts` dispatcher) — interim results display faded, final results trigger translation
6. Build the Claude translation proxy — only translate FINAL segments, never interim
...
User taps Record
  ...
  Processed audio streams via AudioWorklet (Int16 PCM, 40ms frames)
  Frames sent via WebSocket to Speechmatics (enhanced, max_delay 1.5)
    — or to Deepgram (nova-3, endpointing=500, smart_format) when STT_PROVIDER is flipped
  Engine returns real-time transcript (interim + final)
  Final segments sent to /api/translate (Claude Haiku 4.5)
...
- Audio pipeline is mandatory. Never send raw mic audio to the STT engine. Always process through the Web Audio API chain. Speaker audio without processing will produce terrible transcription.
...
- Auto-reconnect the STT WebSocket on disconnect, re-fetching credentials each attempt.
...
1. Capture mic, process through audio pipeline, stream to the STT engine, get transcript, translate, display
2. Test with Arabic audio playing through a speaker (not headphones, not direct speech — an actual speaker in the room)
3. If accuracy is poor, compare engines head-to-head at /dev/stt-compare rather than guessing at model names
